...
gtsam::Point3 triangulateCauchy(const gtsam::CameraSet<gtsam::PinholeCamera<gtsam::Cal3_S2>> &poses,
                                ...
                                const gtsam::SharedNoiseModel &measurementNoise)
{
    ...
    double sigma = measurementNoise->sigmas().mean();
    ...
    for (size_t i = 0; i < m; i++)
    {
        ...
        // \tilde S^{-1} = [S^{-1}              -S^{-1}@m]
        //                  -m.T@S^{-1}  1 + m.T@S^{-1}@m]
        auto measurement = calibratedMeasurements[i];
        ...
        const gtsam::Matrix33 Ai = poses[i].rotation().matrix();
        const gtsam::Matrix33 invA_S_invAT = Ai.transpose() * hS * Ai;
        const gtsam::Matrix33 AT_invS_A = Ai.transpose() * ihS * Ai;
